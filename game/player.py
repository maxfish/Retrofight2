...
class Player(Character):
    ...
    def __init__(self, id, world, game_controller, player_name, footprint_dim=Character.DEFAULT_FOOTPRINT_DIM):
        self.frames_store = FramesStore()
        self.frames_store.load('resources/sprites/' + player_name, 'sprites.json')
        self._shadow_sprite = Sprite(self.frames_store)

        # TODO: The face should be a frame within the sprite definition
        self.face = Texture.load_from_file('resources/sprites/' + player_name + '/face.png')

        super().__init__(world, self.frames_store, 0, 0, footprint_dim)

        self.id = id
        self.joy = game_controller
        ...
        self._shadow_sprite.shader = Shader.from_files('resources/shaders/base.vert',
                                                       'resources/shaders/character_shadow.frag')
    ...
    def update(self, game_speed):
        ...
        self._shadow_sprite.update(game_speed)

        super().update(game_speed)

    def draw(self, screen):
        self._shadow_sprite.x = self.position.x + self._world.window_x
        self._shadow_sprite.y = self.position.y + self._z + self._world.window_y
        self._shadow_sprite.draw(screen)
...
class PlayerState(Entity.State):
    ...
    def play_anim(self, anim_name, loop=False, speed=1):
        ...
        self.p._shadow_sprite.play_animation(anim_name, FramesStore.FLAG_FLIP_Y | (
            FramesStore.FLAG_FLIP_X if self.p.direction == Character.DIR_LEFT else 0) |
                                             (FramesStore.FLAG_LOOP_ANIMATION if loop else 0), speed=speed)
        # self.p._shadow_sprite.play_animation('shadow')
    ...
    def handle_input(self):
        ...
        if self.p._world.scene == self.p._world.SCENE_GAME_OVER:
            if self.p._world.game_over_state >= 10:
                if self.p.joy.is_button_pressed(GameController.BUTTON_A) or self.p.joy.is_button_pressed(
                        GameController.BUTTON_B):
        ...
        if self.p.joy.is_button_down(GameController.BUTTON_DPAD_RIGHT) or \
                self.p.joy.is_button_down(GameController.BUTTON_DPAD_LEFT) or \
                self.p.joy.is_button_down(GameController.BUTTON_DPAD_DOWN) or \
                self.p.joy.is_button_down(GameController.BUTTON_DPAD_UP):
...
class PlayerStateWalk(PlayerState):
    ...
    def handle_input(self):
        ...
        if not self.p.joy.is_button_down(GameController.BUTTON_DPAD_LEFT) and not self.p.joy.is_button_down(
                GameController.BUTTON_DPAD_RIGHT) \
                and not self.p.joy.is_button_down(GameController.BUTTON_DPAD_UP) and not self.p.joy.is_button_down(
            GameController.BUTTON_DPAD_DOWN):
            return self.p.STATE_STAND

        anim_speed = 1

        if self.p.joy.is_button_down(GameController.BUTTON_DPAD_RIGHT):
            self.p.direction = self.p.DIR_RIGHT
            self.p.move_v.x = self.p.WALK_H_SPEED
        elif self.p.joy.is_button_down(GameController.BUTTON_DPAD_LEFT):
            self.p.direction = self.p.DIR_LEFT
            self.p.move_v.x = -self.p.WALK_H_SPEED
        elif self.p.joy.is_button_down(GameController.BUTTON_DPAD_DOWN):
            self.p.move_v.y = self.p.WALK_V_SPEED
            anim_speed = 0.4
        elif self.p.joy.is_button_down(GameController.BUTTON_DPAD_UP):
            self.p.move_v.y = -self.p.WALK_V_SPEED
            anim_speed = 0.4
...
class PlayerStatePunch(PlayerState):
    def enter(self, old_state):
        ...
        elif self.p.joy.is_button_down(GameController.BUTTON_B):
...
class PlayerStateJump(PlayerState):
    ...
    def enter(self, old_state):
        ...
        self.long_jump = (
            old_state == self.p.STATE_WALK or self.p.joy.is_button_down(
                GameController.BUTTON_DPAD_RIGHT) or self.p.joy.is_button_down(
                GameController.BUTTON_DPAD_LEFT))
    ...
    def handle_input(self):
        super().handle_input()
        if self.p.joy.is_button_pressed(GameController.BUTTON_A) and self.p.joy.is_button_down(
                GameController.BUTTON_DPAD_DOWN):
            self.p.attack_force = self.p.ATTACK_FORCE_PUNCH_1
            self.play_anim("dive_attack")
        elif self.p.joy.is_button_pressed(GameController.BUTTON_A):
            if self.long_jump:
                self.p.attack_force = self.p.ATTACK_FORCE_PUNCH_1
