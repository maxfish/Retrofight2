...
class FramesStore:
    ...
    def draw_frame(self, surface, frame, destination_x, destination_y, flags):
        ...
        # Gfx.draw_image_at(surface, frame.image, x_pos, y_pos)
        Gfx.draw_anchored_image_at(frame.image, frame.rect, destination_x, destination_y, anchor_x, anchor_y,
                                   flip_x=flip_x)
...
class Frame:
    def __init__(self, data, frame_name):
        ...
        if anchor:
            self.anchor = Vector2d(anchor['x'], anchor['y'])
        else:
            self.anchor = Vector2d(0, 0)
